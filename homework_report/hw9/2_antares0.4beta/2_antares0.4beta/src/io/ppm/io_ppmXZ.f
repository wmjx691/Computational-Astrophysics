...
*==============================================================
       subroutine makemapXZ(den,rgb,nprnt,ihpixf,jvpixf,khpixf,color2)
       implicit none
       include 'fluid.h'
       include 'parameters'
       include 'color.h'
       integer   ihpixf,jvpixf,khpixf
       character*1 rgb(3,ihpixf,khpixf)
       real*8 den(-1:ihpixf-2,-1:jvpixf-2,-1:khpixf-2)
       integer nrho(ihpixf,khpixf)
       integer   m,ii,jj
...
*---   Linear ouput 3D
       if (ilog .eq. 1) then
         ...
         do k = -1,khpixf-2
         do i = -1,ihpixf-2
           if (rhomin .gt. den(i,jend/2,k)) then
             rhomin = den(i,jend/2,k)
           endif
           if (rhomax .lt. den(i,jend/2,k)) then
             rhomax = den(i,jend/2,k)
           endif
         enddo
         enddo
...
*--------------------------------
       ...
       do k = -1,khpixf-2
       do i = -1,ihpixf-2
       nrho(i+2,k+2)= ANINT( (den(i,jend/2,k)-rhomin)/drho )      
         if (rhomax.eq.rhomin)then
               nrho(i+2,k+2)=128
         endif      
         if (nrho(i+2,k+2) .gt. 256)then
               nrho(i+2,k+2)=256
         elseif(nrho(i+2,k+2).lt.1)then 
               nrho(i+2,k+2)=1
         endif    
       enddo
       enddo
       ...
       do j=1,khpixf
        m = nrho(i,j)
c        m = nrho(j,i)
c       rgb(k,j,i)= char(color2(k,m))
       rgb(k,i,j)= char(color2(k,m))
       enddo
       enddo
       enddo
*========================================
*      Log scale output 3D
       ...
       do k = -1,khpixf-2
       do i = -1,ihpixf-2
         if (rhomin .gt. dlog(den(i,jend/2,k))) then
           rhomin = dlog(den(i,jend/2,k))
         endif
         if (rhomax .lt. dlog(den(i,jend/2,k))) then
           rhomax = dlog(den(i,jend/2,k))
         endif
       enddo
       enddo
*---  Fixed rhomin and rhomax ---
...
*--------------------------------
       ...
       do k = -1,khpixf-2
       do i = -1,ihpixf-2
        nrho(i+2,k+2)= ANINT( (dlog(den(i,jend/2,k))-rhomin)/drho )    
         if (rhomax.eq.rhomin)then
               nrho(i+2,k+2)=128
         endif      
         if (nrho(i+2,k+2) .gt. 256)then
...
* --------------------------------------------
*
       subroutine pixoutXZ(rgb,nframe,ihpixf,jvpixf,khpixf)
       implicit none
* interface arg.
       integer ihpixf, jvpixf, khpixf
       character*1 rgb(3,ihpixf,khpixf)      ! RGB data array
       integer nframe
* local
...
* PPM P6
         ...
         write(fnameout,'(''rXZ'',i4.4,''.ppm'')') nframe ! name of PPM file
...
* header
     ...
     &     ihpixf, khpixf
* image data
         idummy = ihpixf * khpixf * 3
         write(frmtstr,'(''('',i8.8,''A,$)'')') idummy     ! make output "format"
         write(2,fmt=frmtstr)                              ! some compiler may not accept this line.
     &     (((rgb(k,i,j),k=1,3),i=1,ihpixf),j=khpixf,1,-1) ! here, j (vertical address) runs from top to bottom.
         close(2)
...
* header
         write(2,'(A)') 'P3'
         write(2,'(2(1x,i4),'' 255 '')')  ihpixf, khpixf
         icnt = 0
* image data
         do j = khpixf, 1, -1                              ! here, j (vertical address) runs from top to bottom.
         do i = 1, ihpixf, 1
         do k = 1, 3
...
* image data
         ...
         write(2,fmt=frmtstr)
     &     (((rgb(k,i,j),k=3,1,-1),i=1,ihpixf),j=1,khpixf) ! writing in BGR order, not RGB.
         close(2)

       endif

       return
       end subroutine pixoutXZ
