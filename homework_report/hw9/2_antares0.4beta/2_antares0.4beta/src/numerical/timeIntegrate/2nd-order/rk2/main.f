...
!=================================================================
!---  Boundary Condition        
        call boundary(den2,px2,py2,pz2,ene2,bx2,by2,bz2,
     &                den2,px2,py2,pz2,ene2,bx2,by2,bz2,uini)
!=================================================================
!                                REA 
!=================================================================

        call godnov3d(t,dt,dx,1,
     &                 den2,px2,py2,pz2,ene2,bx2,by2,bz2,
     &                 den2,px2,py2,pz2,ene2,bx2,by2,bz2,
...
!---  SOURCE TERM
        call source(den2,px2,py2,pz2,ene2,bx2,by2,bz2,
     &              den3,px3,py3,pz3,ene3,bx3,by3,bz3,t,dt)
...
!---   checking values -------------------------------
        ...
        do j=jbeg,jend
        do i=ibeg,iend
         c1(i,j,k)=0.5d0*(den(i,j,k)+den3(i,j,k))
         c2(i,j,k)=0.5d0*( px(i,j,k)+ px3(i,j,k))
         c3(i,j,k)=0.5d0*( py(i,j,k)+ py3(i,j,k))
         c4(i,j,k)=0.5d0*( pz(i,j,k)+ pz3(i,j,k))
         if (.not.ISOTHERMAL)then
           c5(i,j,k)=0.5d0*(ene(i,j,k)+ene3(i,j,k))
         endif
         if(MHD)then
           c6(i,j,k)=0.5d0*( bx(i,j,k)+ bx3(i,j,k))
           c7(i,j,k)=0.5d0*( by(i,j,k)+ by3(i,j,k))
           c8(i,j,k)=0.5d0*( bz(i,j,k)+ bz3(i,j,k))
         endif

         if (c1(i,j,k).le. 0.d0) then
           dt = 0.5d0*dt
