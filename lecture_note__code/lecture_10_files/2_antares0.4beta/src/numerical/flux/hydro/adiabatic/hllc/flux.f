...
C     Apr. 2007
C================================================================
      !SUBROUTINE FLUX(UL,UR,slopeL,slopeR,FX,UC,dt,sndL,sndR)
      ...
      pTL = (gam-1.d0)*(eneL-0.5d0*rhoL*(uxL**2+uyL**2+uzL**2))
...
c        write(*,*)  'ooooops L'
      ...
      pTR = (gam-1.d0)*(eneR-0.5d0*rhoR*(uxR**2+uyR**2+uzR**2))
...
c       pysL = (SL-uxL)*rhoL*vxL/(SL-SM)
      ...
      pysL = rhosL*uyL
      pzsL = rhosL*uzL
      enesL = ((SL-uxL)*eneL-pTL*uxL+pstar*SM)/(SL-SM)
...
c       pysR = (SR-uxR)*rhoR*vxR/(SR-SM)
      ...
      pysR = rhosR*uyR
      pzsR = rhosR*uzR
      enesR = ((SR-uxR)*eneR-pTR*uxR+pstar*SM)/(SR-SM)
     ...
     &        /(SR-SL+PT)
       Fs(5)= (SR*FL(5)-SL*FR(5)+SR*SL*(UR(5)-UL(5)))/(SR-SL+PT) 
