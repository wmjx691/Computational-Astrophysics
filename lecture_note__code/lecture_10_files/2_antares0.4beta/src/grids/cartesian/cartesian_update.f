      subroutine cartesian_update(dtdx,coef,cx,cy,cz,nd,
     ...
     &         (F_den(i-cx,j-cy,k-cz)-F_den(i,j,k))*dtdx
       if (den3(i,j,k) .le. smalld) then
               write(*,*)'Warning! low density'
               den3(i,j,k) = smalld
       endif
     ...
     &         (F_ene(i-cx,j-cy,k-cz)-F_ene(i,j,k))*dtdx
       ...
       if (ene3(i,j,k) .le. smalle) then
               write(*,*)'Warning! low energy'
               ene3(i,j,k) = smalle
       endif
        if (MHD)then
