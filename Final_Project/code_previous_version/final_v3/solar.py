...
for time in time_duration:
    ...
    # arrive target planet
        ...
        if (recordint == 0):
            #can back to here
            step_startsoi   = step
            stell_startsoi  = stell
            print("is x:", stell_startsoi.x , "y:", stell_startsoi.y, "z:", stell_startsoi.z)
            ...
            while (closest_result == False):
                ...
                if (closest_result):
                    ...
                    x       = stell_startsoi.x - stars[speedupinfo.idindex].x[step_endsoi]
                    y       = stell_startsoi.y - stars[speedupinfo.idindex].y[step_endsoi]
                    z       = stell_startsoi.z - stars[speedupinfo.idindex].z[step_endsoi]

                    speedupinfo.shortest = distance(stell, stars, speedupinfo.idindex, step)
            ...
            stell          = stell_startsoi

            print("this is x:", stell_startsoi.x , "y:", stell_startsoi.y, "z:", stell_startsoi.z)


            speedupinfo.speed_ox = False
            stell   = update(stell, stars, step, dt, speedupinfo)

            print("This is:",time_duration[step])
            first   = record(stell, time, first, planet)
            step    = step+1
